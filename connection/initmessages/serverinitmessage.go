...
type ServerInitMessage struct {
	// Version of the server-side API. A client is supposed to check if returned API level is
	// greater than or equal to ExpectedServerVersion once a connection has been established in order
	// to ensure that all of the required commands are actually supported by the control server.
	Version int64
